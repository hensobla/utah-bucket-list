...
The tiny `Build <sha>` line at the very bottom of the page is the deployed commit's short hash — a quick way to visually confirm you're actually looking at the latest push rather than something cached. It's hand-set in `index.html`'s `.notes__version`, not generated by a build step (there isn't one), so bump it as part of each commit that ships a real change: commit first, then `git rev-parse --short HEAD`, then edit it in and amend.
