...
GitHub Pages caches every file — including `index.html` itself — for 10 minutes server-side (`cache-control: max-age=600`), and there's no way to override that from a static repo. iOS home-screen installs can also hold onto old content more stubbornly than a regular Safari tab. `sw.js` is a minimal service worker that closes that gap, but getting it right took two fixes, not one — both worth knowing if you ever touch this file:

- **The fetch handler** tries the network first on every same-origin request and only falls back to its own cache if there's no connection at all. But a plain `fetch(req)` inside a service worker still goes through the browser's *own* HTTP cache by default — so it needed `{ cache: "no-store" }` on that fetch specifically to actually bypass GitHub Pages' cache-control instead of quietly honoring it.
- **The registration itself** needed `{ updateViaCache: "none" }` (plus an explicit `registration.update()` on every load) — otherwise the browser checks whether `sw.js` changed using those same HTTP caching rules, so it could go up to 10 minutes without ever noticing a new service worker was deployed, including fixes to this exact freshness logic.

One real caveat, not worth engineering around further: a newly-registered service worker doesn't take control of the page that triggered its own registration (`clients.claim()` picks it up momentarily after activation, or definitely by the next load) — so the very first load after visiting a stale/cached tab may still show the old version for a moment, with the next reload guaranteed fresh. A brand-new visitor, or anyone opening the app after more than 10 minutes since their last visit, never hits this at all.
